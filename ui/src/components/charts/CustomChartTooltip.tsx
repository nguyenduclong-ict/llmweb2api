...
export default function CustomChartTooltip({
  ...
  const total = showShare ? payload.reduce((sum: number, entry: any) => sum + (Number(entry.value) || 0), 0) : 0;
  ...
  return (
    ...
    >
      {displayLabel && <p style={{ margin: '0 0 6px 0', fontWeight: 600, color: '#374151' }}>{displayLabel}</p>}
        ...
        return (
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              />
              <span style={{ color: '#6b7280', whiteSpace: 'nowrap' }}>{entry.name}</span>
            </span>
            <span style={{ fontWeight: 600, color: '#111827', whiteSpace: 'nowrap' }}>
              {entry.unit ? `${formatValue(value, true)} ${entry.unit}` : formatValue(value, true)}
              {sharePercent !== null && (
                <span style={{ fontSize: 10, color: '#9ca3af', marginLeft: 4 }}>({sharePercent}%)</span>
              )}
            </span>
          </div>
        );
